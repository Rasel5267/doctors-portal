...
const verifyAccount = async (req, res) => {
	try {
		...
		if (user.otp === otp) {
			...
			return res
				.status(200)
				.send({ success: false, message: "OTP is not correct" });
		}
	} catch (error) {
		res.status(500).send({ success: false, message: error.message });
	}
...
const applyDoctor = async (req, res) => {
	try {
		...
		sendEmail(
			adminUser.email,
			`${newDoctor.name} has applied for a doctor account`
		);
		await userModel.findByIdAndUpdate(adminUser._id, { notification });

		res.status(200).send({
			message:
				"Application successfully submit. Wait for admin confirmation",
			success: true,
		});
	} catch (error) {
		res.status(500).send({ success: false, message: error.message });
...
const getAvailableAppointments = async (req, res) => {
	try {
		...
		res.status(200).send({
			success: true,
			message: "Appointment list fetch success",
...
const bookAppointment = async (req, res) => {
	try {
		...
		const alreadyBooked = await appointmentModel.find({
			userId: req.body.userId,
		});

		if (alreadyBooked.length >= 2) {
			res.status(200).send({
				success: true,
				message: "You have already 2 appointments",
			});
		} else {
			...
			sendEmail(
				user.email,
				`A new appointment request from ${req.body.userInfo.name}`
			);
			await user.save();
			res.status(200).send({
				success: true,
